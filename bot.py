...
def clean_text(text: str) -> str:
    """Убирает markdown-звёздочки и заменяет все t.me ссылки на MY_CHANNEL."""
    ...
    text = re.sub(r'https?://t\.me/\S+', MY_CHANNEL, text)
    # Убираем @username если это чужой канал (оставляем только MY_CHANNEL)
    # text = re.sub(r'@\w+', MY_CHANNEL, text)  # раскомментируй если нужно
    return text.strip()
